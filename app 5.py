...
def loan_payoff_pdf(lid, pid):
    ...
    if not pdf_bytes:
        ...
        data = {
            ...
            "interest_fee": money(row["interest_fee"]),
            "other_fees": money(row["other_fees"]),
            "per_diem": money(row["per_diem"]),
            "total": money(row["total"]),
            "wire_bank": row["wire_bank"],
            "wire_name": row["wire_name"],
            "wire_routing": row["wire_routing"],
            "wire_account": row["wire_account"],
            "wire_further": row["wire_further"],
            "notes": row["notes"],
        }
        extra = payoff_defaults(loan)
        for k, v in extra.items():
            data.setdefault(k, v)
        pdf_bytes = payoff_letter_pdf(loan, b, data)
        stored = row["filename"] or f"payoff_{lid}_{pid}.pdf"
        try:
            os.makedirs(UPLOAD_DIR, exist_ok=True)
            with open(os.path.join(UPLOAD_DIR, stored), "wb") as fh:
                fh.write(pdf_bytes)
            if not row["filename"]:
                db().execute("UPDATE payoff_letters SET filename=? WHERE id=?", (stored, pid))
                db().commit()
        except OSError:
            pass
    return send_file(
