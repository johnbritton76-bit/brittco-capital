...
def loan_alerts():
    ...
    for r in rows:
        ...
        if mark in seen:
            continue
        seen.add(mark)
        _base, mat_date = loan_term_dates(r)
        stored = row_val(r, "maturity_date")
        if stored and (not mat_date or (parse_date(stored) and parse_date(mat_date) and parse_date(stored) > parse_date(mat_date))):
            mat_date = stored
        mat = days_until(mat_date)
        name = r["borrower_name"]
        label = num or f"Loan {lid}"
        items = []
        if mat is not None and 0 <= mat <= 14:
            items.append({
                "level": "warn",
                "kind": "maturity",
